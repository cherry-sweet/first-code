...
def main():
    ...
    # Load dataset
    ##################################
    # dog: train_dataset: DogDataset:12000    validate_dataset:DogDataset:8580
    train_dataset, validate_dataset = get_trainval_datasets(config.tag, config.image_size)
    # train_loader:DataLoader:188   validate_loader:DataLoader:34
    ...
    # feature_center: size of (#classes, #attention_maps * #channel_features)   特征中心初始化  tensor:(120,65536)
    feature_center = torch.zeros(num_classes, config.num_attentions * net.num_features).to(device)
    # net.num_features:2048  config.num_attentions = 32
    if config.ckpt:
    ...
    ##################################
    learning_rate = logs['lr'] if 'lr' in logs else config.learning_rate    # 0.001
    ...
    # ModelCheckpoint
    ##################################
    callback_monitor = 'val_{}'.format(raw_metric.name)    # str:val_topk_accuracy
...
def train(**kwargs):
    ...
    for i, (X, y) in enumerate(data_loader):
        ...
        X = X.to(device)   # tensor(64,3,448,448)
        y = y.to(device)   # tensor(64)

        ##################################
        # Raw Image  原始图像
        ##################################
        # raw images forward   y_pred_raw:tensor(64,120)  feature_matrix:tensor(64,65536)  attention_map:tensor(64,2,14,14)
        y_pred_raw, feature_matrix, attention_map = net(X)

        # Update Feature Center  更新特征中心
        feature_center_batch = F.normalize(feature_center[y], dim=-1)
        feature_center[y] += config.beta * (feature_matrix.detach() - feature_center_batch)
        # feature_center:tensor(120,65536)  y:tensor(64)
        ##################################
        # Attention Cropping
        ##################################
        with torch.no_grad():    #x:tensor(64,3,448,448)  attention_map:tensor(64,2,14,14)
