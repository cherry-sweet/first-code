...
def main():
    ...
    with torch.no_grad():
        ...
        pbar.close()
        with open('/media/data/zpp/code/wsdan/FGVC/dogs/ckpt/eval.txt', 'a') as f:
            f.write(str(epoch_raw_acc[0]) + '\n')
            f.write(str(epoch_raw_acc[1]) + '\n')
            f.write(str(epoch_ref_acc[0]) + '\n')
            f.write(str(epoch_ref_acc[1]) + '\n')
            f.write("#######################")
